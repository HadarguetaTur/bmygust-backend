...
async function query(filterBy = { dest: "", price: 0 ,labels:""}) {
  try {
    const collection = await dbService.getCollection("stay")
    var stays = await collection.find().toArray()

    if (!filterBy.dest && filterBy.price&& !filterBy.labels) return stays
    var newStays = stays
    ...
    if (filterBy.labels) {
      newStays = stays.filter((stay) => stay.labels[0] === filterBy.labels)
    }
